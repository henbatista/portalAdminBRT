...
const updateCountryId = (CountryName: string, newCountryId: any) => {
  ...
  stateStore.name = CountryName;
};


function handleCountry() {
  if (stateStore.idDeleteOrUpdate === 0) {
    stateStore.saveStates(stateStore.name, country.value.country_id);
  } else {
    stateStore.updateStates(stateStore.name, country.value.country_id);
  }
}
...
<template>
  ...
  <div class="bg-slate-50 justify-center p6 -mx-6 px-6 py-6">
  ...
  <section class="flex p mt-4 p-6 flex-col gap-4">
    ...
    <AutoComplete :countryId="country.country_id" :updateCountryId="updateCountryId" />
    ...
    @click="handleCountry"
      ...
      >
        ...
        <span v-if="stateStore.idDeleteOrUpdate === 0">ADICIONAR PÁIS</span
