...
const getAllTenants = async () => {
  try {
    ...
    const { data } = await axios.get(
      `${apiUrl}/api/v1/cities/pluck?search=${query.value}&order=asc&page=1&per_page=10`,
      {
        headers: {
    ...
    toast.error("API fora do ar, entre em contato com a BRT!");
  }
};
...
watch(selected, (newVal) => {
  ...
  props.updateTenantId(
    newVal.map((item) => item.name),
    tenant_ids.value,
  );
});
...
onMounted(() => {
  if (markupStore.markupToUpdate !== null) {
    console.log("Dudu", markupStore.markupToUpdate.commissions);
    selected.value = markupStore.markupToUpdate.commissions;
  }
});
</script>
...
<template>
  <div class="w-full">
    <Combobox v-model="selected" multiple>
      <span class="text-sm">Cidades Próximas</span>
      ...
      <div class="relative mt-1">
        <ComboboxInput
          ...
          placeholder="Quais cidades são próximas a este aeroporto?"
          @input="updateQuery"
        />
        <ComboboxButton
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm border-collapse">
          <thead>
            <tr>
              <th
                class="px-2 py-2 border border-gray-900 text-white bg-slate-900"
              >
                Empresa
              </th>
              <th
                class="px-2 py-2 border border-gray-900 text-center text-white bg-slate-900"
              >
                Excluir
              </th>
              <!-- Coluna adicional para ações -->
            </tr>
          </thead>
          <tbody>
            <tr
