...
function handleSubmit() {
  // Atribuição por desconstrução para obter idDeleteOrUpdate de tenantStore
  const { idDeleteOrUpdate } = tenantStore;

  // Verificando se é uma operação de atualização com base na presença de idDeleteOrUpdate
  const isUpdate = idDeleteOrUpdate !== "";

  // Determinando se deve chamar updateTenant ou saveTenant com base em isUpdate
  const saveOrUpdateTenant = isUpdate
    ? tenantStore.updateTenant
    : tenantStore.saveTenant;

  // Chamando a função apropriada (saveTenant ou updateTenant) com os dados do formulário
  saveOrUpdateTenant(
    tenantStore.idDeleteOrUpdate, // assumindo que idDeleteOrUpdate é usado aqui
    tenantStore.slug_id,
  ...
  switch (field) {
    ...
    case "corporate_name":
      showMinLengthWarning.corporate_name =
        formTenant.value.corporate_name.length < minLength;
      break;
    default:
      break;
  }
...
onMounted(() => {
  const tenantExist = tenantStore.tenants?.find(
    (tenant) => tenant.id === tenantStore.idDeleteOrUpdate,
  );
  if (tenantExist) {
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-4 px-6">
    ...
    <div class="grid gap-5 text-slate-900 md:grid-cols-2 grid-cols-1">
      <div>
      ...
      <div>
        <div class="sm:col-span-3 md:mt-1 -mt-4">
          <label
            for="street-address"
            class="flex-0 mr-6 md:w-[100px] w-[60px] break-words input-label"
          >
          ...
          <div class="mt-2" v-if="estimateSales && estimateSales.length > 0">
            <select
              id="agency-estimates"
      ...
      <div>
        <div
          class="sm:col-span-3 md:mt-0 -mt-4"
        ...
        <div class="mt-0">
          ...
          <div class="mt-2 mb-4">
            <select
              id="type-account"
              name="type-account"
              v-model="formTenant.tenant_type"
              autocomplete="type-account"
              class="block w-full rounded-sm b py-2.5 g-white transition duration-300 ease-in-out border border-slate-200 focus:ring-0 focus:outline-none text-slate-900 text-sm px-3 placeholder:font-light focus:border-slate-600"
            >
        ...
        <div class="grid gap-1 text-slate-900 md:grid-cols-1 grid-cols-1 mb-2">
          ...
          >
            ...
            <span
              aria-hidden="true"
              :class="
                formTenant.is_active ? 'translate-x-4' : '-translate-x-0.5'
              "
      ...
      <div class="flex justify-end gap-1">
        ...
        >
          ...
          >
            ...
            <span v-if="tenantStore.idDeleteOrUpdate === ''"
