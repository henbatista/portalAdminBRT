...
<template>
  <div :id="props.inputId" class="mt-4 flex flex-col gap-2">
    <p class="mt-1 text-sm font-bold">
      {{ props.label }}
      <span v-if="props.mandatory" class="text-red-500 text-xs"
        >(Obrigatório)</span
      >
    </p>
    <div class="relative w-full flex">
      ...
      <label
        :for="props.inputId"
        @click="() => triggerFileInput(props.inputRef)"
        class="w-full h-[40px] file-control flex items-center rounded-sm b g-white transition duration-300 ease-in-out border border-slate-200 focus:ring-0 focus:outline-none t text-sm pl-3 placeholder:font-light focus:border-slate-600 cursor-pointer bg-slate-100 text-slate-900 text-center px-4 py-2 rounded-l-lg"
      >
        Buscar arquivo
      </label>
      <span
        :id="props.fileNameId"
        class="p-2 border w-2/3 h-10 flex-none cursor-pointer border-l px-4 border-slate-200 inline-flex items-center text-slate-600 text-base rounded-tr rounded-br font-normal"
        >{{ props.fileName }}</span
      >
    </div>
